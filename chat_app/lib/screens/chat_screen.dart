...
class __DateLableState extends State<_DateLable> {
  ...
  @override
  void initState() {
    final createdAt = widget.dateTime;
    final now = DateTime.now();

    if (Jiffy.parseFromDateTime(createdAt).isSame(Jiffy.now(), unit:Unit.day)) {
      dayInfo = 'TODAY';
    } else if (Jiffy.parseFromDateTime(createdAt)
        .isSame(Jiffy.parseFromDateTime(now.subtract(const Duration(days: 1))),unit:Unit.day)) {
      dayInfo = 'YESTERDAY';
    } else if (Jiffy.parseFromDateTime(createdAt).isAfter(
      Jiffy.parseFromDateTime(now.subtract(const Duration(days: 7))),
      unit:Unit.day,
    )) {
      dayInfo = Jiffy.parseFromDateTime(createdAt).EEEE;
    } else if (Jiffy.parseFromDateTime(createdAt).isAfter(
      Jiffy.parseFromDateTime(now).subtract(years: 1),
      unit:Unit.day,
    )) {
      dayInfo = Jiffy.parseFromDateTime(createdAt).MMMd;
    } else {
      dayInfo = Jiffy.parseFromDateTime(createdAt).MMMd;
    }
